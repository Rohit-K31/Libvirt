              ...
              gpointer opaque)
{
    ...
    if (mon->lastError.code != VIR_ERR_OK) {
        ...
        if (!error && !mon->goteof &&
            cond & G_IO_ERR) {
            virReportError(VIR_ERR_INTERNAL_ERROR,
                           _("Invalid file descriptor while waiting for monitor vm=%p name=%s"),
                           vm, vm->def->name);
            mon->goteof = true;
        }
    }
                ...
                qemuMonitorMessage *msg)
{
    ...
    if (mon->lastError.code != VIR_ERR_OK) {
        VIR_DEBUG("Attempt to send command while error is set %s vm=%p name=%s",
                  NULLSTR(mon->lastError.message), vm, vm->def->name);
        virSetError(&mon->lastError);
        return -1;
    }
    if (mon->goteof) {
        virReportError(VIR_ERR_INTERNAL_ERROR,
                       _("End of file from qemu monitor vm=%p name=%s"),
                       vm, vm->def->name);
        return -1;
    }
    ...
    while (!mon->msg->finished) {
        if (virCondWait(&mon->notify, &mon->parent.lock) < 0) {
            virReportError(VIR_ERR_INTERNAL_ERROR,
                           _("Unable to wait on monitor condition vm=%p name=%s"),
                           vm, vm->def->name);
            goto cleanup;
        }
    }
